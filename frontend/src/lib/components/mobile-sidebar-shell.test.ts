...
const layout = await Bun.file(`${import.meta.dir}/../../routes/(app)/+layout.svelte`).text();
const sidebar = await Bun.file(`${import.meta.dir}/sidebar/Sidebar.svelte`).text();
const toggle = await Bun.file(`${import.meta.dir}/MobileSidebarToggle.svelte`).text();
const host = await Bun.file(`${import.meta.dir}/../hosts/DocsmintAppShellHost.svelte`).text();
...
describe('responsive app shell', () => {
  ...
  test('provides an invisible touch-safe edge swipe zone', () => {
    expect(host).toContain('data-mobile-sidebar-edge');
    expect(host).toContain('w-9');
    expect(host).toContain('touch-action: pan-y');
    expect(host).toContain('createEdgeSwipeTracker');
    expect(host).toContain('openMobileSidebar()');
  });

  test('keeps the mobile drawer accessible and restores focus', () => {
    expect(sidebar).toContain('role={mobile && mobileOpen ? "dialog" : undefined}');
