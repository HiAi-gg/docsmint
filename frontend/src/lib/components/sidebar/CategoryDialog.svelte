...
<Dialog bind:open onOpenChange={handleDialogOpenChange}>
      ...
      {#if createdCategoryKey}
        <div class="rounded-md border border-primary/20 bg-background/80 p-3">
          <p class="text-xs font-medium text-foreground">
            Copy the API key now. It remains available in Settings → API.
          </p>
          <code class="mt-2 block break-all rounded bg-muted p-2 text-xs text-foreground"
            >{createdCategoryKey}</code
          >
          <Button
            type="button"
            size="sm"
            variant="outline"
            class="mt-2"
            onclick={() => navigator.clipboard.writeText(createdCategoryKey ?? '')}
          >
  ...
  {:else if !isDeleteMode}
    ...
    >
      <div class="space-y-2">
        ...
        />
        {#if error}
          <p id="category-dialog-name-error" class="text-xs text-destructive" role="alert">
            {error}
          </p>
        {/if}
      </div>
      ...
      {#if apiMode !== 'unavailable'}
        <div class="space-y-3 rounded-md border border-border/70 bg-muted/30 p-3">
          <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex flex-wrap gap-2">
              <Button
                ...
                variant="outline"
                size="sm"
                onclick={() => applyPermissionPreset('read-write')}
              >
                Read / Write
              </Button>
              <Button
                ...
                variant="outline"
                size="sm"
                onclick={() => applyPermissionPreset('read-edit-write')}
              >
                Read / Edit / Write
              </Button>
            </div>
          ...
          <div class="grid gap-2 sm:grid-cols-3">
            <label
              class="flex items-center gap-2 rounded-md border border-border/60 bg-background px-3 py-2 text-sm"
            >
              <input
                bind:checked={apiPermissionRead}
                disabled={busy}
                type="checkbox"
            ...
            >
              <input
                bind:checked={apiPermissionEdit}
                disabled={busy}
                type="checkbox"
                class="size-4 rounded border-input"
              />
            ...
            >
              <input
                bind:checked={apiPermissionWrite}
                disabled={busy}
                type="checkbox"
                class="size-4 rounded border-input"
              />
      ...
      {#if mode === 'edit' && apiMode === 'category' && category?.id}
        <div class="space-y-3 rounded-md border border-border/70 p-3">
          <div class="flex items-center justify-between gap-2 max-sm:flex-col max-sm:items-stretch">
            <div>
              <Label>Category API keys</Label>
              <p class="text-xs text-muted-foreground">
                The current permissions are saved before the key is issued.
              </p>
            </div>
            <Button
              type="button"
              ...
              disabled={busy || keyBusy || !trimmedName || !hasAnyPermission}>Create key</Button
            >
          </div>
          {#if latestIssuedKey}
            <div class="rounded-lg border border-primary/30 bg-primary/10 p-3 text-primary">
              ...
              <code
                class="mt-1 block break-all rounded bg-background/80 p-2 text-xs text-foreground"
                >{latestIssuedKey}</code
              >
              <Button
                type="button"
                size="sm"
                variant="outline"
                class="mt-2"
                onclick={() => navigator.clipboard.writeText(latestIssuedKey)}>Copy key</Button
              >
            </div>
          {/if}
              ...
              >
                ...
                <div class="flex shrink-0 gap-2">
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    onclick={() => copyCategoryKey(key)}
                    disabled={!key.recoverable && !issuedKeys[key.id]}
                    >{key.recoverable || issuedKeys[key.id]
                      ? 'Copy key'
                      : 'Rotate to enable copy'}</Button
                  >
                  <Button
                    type="button"
                    size="sm"
                    variant="destructive"
                    onclick={() => revokeCategoryKey(key.id)}
                    disabled={keyBusy}>Revoke</Button
                  >
                </div>
              </div>
            {/each}
  ...
  <DialogFooter class="gap-2 max-sm:flex-col max-sm:items-stretch">
    ...
    <Button
      ...
      disabled={busy ||
        (!isDeleteMode && !createdCategoryName && !deletedCategoryName && trimmedName.length === 0)}
    >
      {#if busy}
