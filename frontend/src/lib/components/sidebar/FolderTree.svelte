...
<script lang="ts">
  ...
  import { createFolder, deleteFolder, listFolders, updateFolder } from '$lib/api/folders';
  ...
  import { getDocRefreshNonce, getSelectedTag, refreshDocs } from '$lib/stores/tag-store.svelte';
   ...
   */
  type DocZone =
    { kind: 'folder'; id: string } | { kind: 'category'; id: string } | { kind: 'root' };
  ...
  function scheduleFolderExpand(folderId: string) {
    ...
    expandTimer = setTimeout(() => {
      if (pendingExpandFolderId === folderId && !expandedFolderIds.has(folderId)) {
  ...
  async function loadFolders() {
    try {
      ...
      folders = (result[0]?.children ?? []).map((f) => {
        registerFolder(f.id, f.parentId ?? null, f.categoryId ?? null, f.order ?? 0);
        return {
          id: f.id,
          name: f.name,
  ...
  async function loadCategories() {
    try {
      const loaded = (await listCategories(request.fetch)) as CategoryWithApiAccess[];
      const ids = new Set<string>();
      for (const category of loaded) {
        if (ids.has(category.id)) {
          throw new Error(`Categories response contains duplicate id: ${category.id}`);
        }
        ids.add(category.id);
      }
      categories = loaded;
  ...
  async function loadDocuments() {
    ...
    try {
      ...
      const res = await listDocuments({ limit: 100, ...(tag ? { tag } : {}) }, request.fetch);
  ...
  function rollbackDocumentPlacement(documentId: string, placement: SidebarDocumentPlacement) {
    ...
    documents = documents.map((doc) => (doc.id === documentId ? { ...doc, ...placement } : doc));
    resyncZonesFromDocuments();
  }

  const writeDocumentPlacement = createDocumentPlacementWriter({
    patch: (documentId, placement) => updateDocument(documentId, placement, request.fetch),
    optimistic: (documentId, placement) =>
      publishDocumentPlacement(documentId, placement.folderId, placement.categoryId),
  ...
  function persistDocumentPlacement(documentId: string, placement: SidebarDocumentPlacement) {
    const original = getDocumentFromRegistry(documentId);
    if (!original) return;
    if (original.folderId === placement.folderId && original.categoryId === placement.categoryId) {
      return;
    }
    void writeDocumentPlacement(documentId, placement, original).catch(() => undefined);
  }
  ...
  function handleConsider(zone: DocZone) {
    return (e: CustomEvent<DndEvent<DndDoc>>) => {
      ...
      if (e.detail.info?.id) {
        ...
        if (startsNewDrag) {
          documentDropCoordinator.begin(e.detail.info.id, ++documentDragGeneration);
        }
      }
  ...
  function handleDragOver(e: DragEvent) {
    if (draggedDocId ?? documentDropCoordinator.pendingId(documentDragGeneration)) {
      e.preventDefault();
      if (e.dataTransfer) {
  ...
  function handleDropOnCategory(e: DragEvent, categoryId: string) {
    const documentId = draggedDocId ?? documentDropCoordinator.pendingId(documentDragGeneration);
  ...
  async function persistFolderChanges(zoneKey: string, zoneItems: FolderItem[]) {
    ...
    const blocked: Set<string> = isParentZone ? computeBlockedAncestors(targetParentId) : new Set();
    ...
    if (isParentZone) {
      for (const folder of zoneItems) {
        ...
        if (original.parentId !== targetParentId) {
          ...
          registerFolder(folder.id, targetParentId, original.categoryId, original.order);
        }
      }
    } else {
      for (const folder of zoneItems) {
        ...
        if (categoryChanged || parentChanged) {
          const payload: typeof updatesMap extends Map<string, infer V> ? V : never = {
            folderId: folder.id,
          };
    ...
    zoneItems.forEach((folder, index) => {
      ...
      if (existingUpdate) {
        ...
        registerFolder(
          folder.id,
          existingUpdate.parentId !== undefined ? existingUpdate.parentId : original.parentId,
          existingUpdate.categoryId !== undefined ? existingUpdate.categoryId : original.categoryId,
          index
        );
    ...
    try {
      ...
      setDndError(
        err instanceof Error ? `Move failed: ${err.message}` : 'Move failed: unknown error'
      );
    }
  }
  ...
  function handleCategoryConsider(e: CustomEvent<DndEvent<CategoryBucket>>) {
    ...
    orderedBuckets = withUncategorizedBucket(validCategoryDndItems(e.detail.items));
  }

  function handleCategoryFinalize(e: CustomEvent<DndEvent<CategoryBucket>>) {
    e.stopPropagation();
    const next = withUncategorizedBucket(finalizeCategoryDndItems(e.detail.items));
    orderedBuckets = next;
    isDraggingGlobal = false;
  ...
  function finalizeCategoryDndItems(raw: unknown): CategoryBucket[] {
    ...
    const shadowIds = new Set(
      items.filter((item) => item[SHADOW_ITEM_MARKER_PROPERTY_NAME]).map((item) => item.id)
    );
    ...
    for (const item of items) {
      ...
      if (shadowIds.has(item.id) && !item[SHADOW_ITEM_MARKER_PROPERTY_NAME]) continue;
  ...
  const uncatBucket = $derived(orderedBuckets.find((b) => b.id === UNCATEGORIZED_KEY) ?? null);
...
{#snippet copyButton(doc: DndDoc)}
  <button
    type="button"
    class="sidebar-touch-action inline-flex size-6 shrink-0 items-center justify-center rounded-md text-muted-foreground opacity-0 transition-opacity hover:bg-accent hover:text-accent-foreground group-hover/doc:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring {copiedDocId ===
      doc.id || copyLoadingDocId === doc.id
      ? 'opacity-100'
      : ''}"
...
<div class="space-y-1">
  <div class="mb-2 flex items-center justify-between gap-1">
    <a
      ...
      title={m.dashboard_title()}>{m.sidebar_folders()}</a
    >
    <button
      type="button"
      ...
      >
        <div class="flex w-full min-w-0 items-center gap-0.5">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onSelect={() => navigateDocsmintRoute(route, `/?category=${bucket.category.id}`)}
              >
                {m.action_go_to()}
              </DropdownMenuItem>
              <DropdownMenuItem
                onSelect={() =>
                  navigateDocsmintRoute(route, `/docs/new?category=${bucket.category.id}`)}
              >
          ...
          <div class="ml-0.5 space-y-0.5 border-l border-border pl-0.5">
            <div
              class={cn(
                ...
                isDraggingFolder &&
                  zoneItems.length === 0 &&
                  'min-h-[36px] bg-accent/20 rounded border border-dashed border-muted-foreground/20'
              )}
              use:dndzone={{
                items: zoneItems,
                flipDurationMs: FLIP_MS,
            ...
            >
              {#each rootDocs as doc (doc.id)}
                <div
                  animate:flip={{ duration: DOCUMENT_FLIP_MS }}
...
<DeleteDialog
  ...
  successTitle={deleteTarget?.kind === 'folder'
    ? m.folders_delete_success()
    : m.doc_delete_success()}
