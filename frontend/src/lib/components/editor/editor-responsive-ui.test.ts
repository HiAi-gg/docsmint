...
describe('responsive editor and settings UI', () => {
  test('embedded toolbar follows the rounded editor container', () => {
    const source = readFileSync(resolve(import.meta.dir, 'EditorToolbar.svelte'), 'utf8');
  ...
  test('settings tabs share the entire available width', () => {
    const source = readFileSync(resolve(import.meta.dir, '../SettingsDialog.svelte'), 'utf8');
  ...
  test('mobile scroll-to-top control clears the floating document editor', () => {
    const source = readFileSync(resolve(import.meta.dir, '../ScrollToTop.svelte'), 'utf8');
  ...
  test('floating toolbar becomes edge-to-edge and wraps on narrow screens', () => {
    ...
    expect(source).toContain('max-height: min(60dvh, 360px)');
    expect(source).toContain('max-width: calc(100vw - 16px)');
  });

  test('keeps touch actions discoverable without changing desktop hover', () => {
    const recentDocs = readFileSync(
      resolve(import.meta.dir, '../sidebar/RecentDocs.svelte'),
      'utf8'
    );
    const folderTree = readFileSync(
      resolve(import.meta.dir, '../sidebar/FolderTree.svelte'),
      'utf8'
    );
  ...
  test('large documents show an animated loading state while parsing', () => {
    const source = readFileSync(resolve(import.meta.dir, 'HiAiEditor.svelte'), 'utf8');
