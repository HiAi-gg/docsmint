<script lang="ts">
  ...
  import { hasMutationRefreshImpact, mutationRefreshImpact } from './mutation-refresh';
  import { provideDocsmintRealtimeAdapter } from './realtime-context';
  import { provideDocsmintRequestAdapter, provideDocsmintRouteAdapter } from './route-context';
  import type {
  ...
  $effect(() => {
    ...
    const previouslyFocused =
      document.activeElement instanceof HTMLElement ? document.activeElement : toggleButton;
    const previousBodyOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    const focusableElements = () =>
      Array.from(sidebarElement?.querySelectorAll<HTMLElement>(focusableSelector) ?? []).filter(
