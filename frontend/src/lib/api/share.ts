...
export function createShareLink(data: CreateShareLinkInput): Promise<ShareLink> {
  return apiFetch("/api/share", { method: "POST", body: JSON.stringify(data) });
}

export function listShareLinks(params?: { documentId?: string }): Promise<{ links: ShareLink[] }> {
...
export function addGuest(linkId: string, email: string): Promise<ShareLink> {
  return apiFetch(`/api/share/${linkId}/guests`, { method: "POST", body: JSON.stringify({ email }) });
}
