...
export interface PipelineBenchmarkReport {
	...
	failureRate: number;
	fairness: { ownerCount: number; minCompleted: number; maxCompleted: number; spread: number };
	providerCalls: number;
}
...
export function buildPipelineBenchmarkReport(
	...
	for (const sample of samples) {
		if (!sample.failed) completedByOwner.set(sample.ownerId, (completedByOwner.get(sample.ownerId) ?? 0) + 1);
		else if (!completedByOwner.has(sample.ownerId)) completedByOwner.set(sample.ownerId, 0);
	}
	...
	return {
		...
		throughputPerMinute: durationMs > 0 ? (samples.length * 60_000) / durationMs : 0,
		queueWaitMs: { p50: percentile(queueWait, 0.5), p95: percentile(queueWait, 0.95) },
		processingMs: { p50: percentile(processing, 0.5), p95: percentile(processing, 0.95) },
		failureRate: samples.length ? samples.filter((sample) => sample.failed).length / samples.length : 0,
		fairness: { ownerCount: counts.length, minCompleted, maxCompleted, spread: maxCompleted - minCompleted },
		providerCalls: samples.reduce((total, sample) => total + (sample.providerCalls ?? 0), 0),
	};
}

if (import.meta.main) {
	console.log("Use buildPipelineBenchmarkReport(samples, durationMs) from the benchmark harness.");
}
