...
export const searchRoutes = new Elysia({ prefix: "/api/search" })
  .get("/", async ({ query, set, request }) => {
    ...
    try {
      ...
      type RawSearchResult = { id: string; title: string; snippet: string; score: number; folder_id: string | null; created_at: string; updated_at: string };
      type SearchResult = { id: string; title: string; snippet: string; score: number; folderId: string | null; createdAt: string; updatedAt: string };
      ...
      const allResults = Array.from(merged.values()).sort((a, b) => b.score - a.score);
