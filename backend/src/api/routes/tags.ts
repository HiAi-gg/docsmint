...
import { tags, documentTags, documents } from "@hiai-docs/db/schema";
...
export const tagRoutes = new Elysia({ prefix: "/api" })
  ...
  .post("/tags", async ({ request, set }) => {
    ...
    try {
      ...
      const [created] = await db
        .insert(tags)
        .values({ ownerId: userId, name: body.data.name, color: body.data.color ?? null })
        .returning();
      set.status = 201;
      return created;
    } catch (err) {
  ...
  .delete("/tags/:id", async ({ params, set, request }) => {
    ...
    if (!userId) { set.status = 401; return { error: "Unauthorized" }; }
    try {
      await db.delete(tags).where(and(eq(tags.id, params.id), eq(tags.ownerId, userId)));
      return { success: true };
    } catch (err) {
  ...
  .post("/documents/:id/tags", async ({ params, request, set }) => {
    ...
    try {
      const [doc] = await db.select({ id: documents.id }).from(documents)
        .where(and(eq(documents.id, params.id), eq(documents.ownerId, userId))).limit(1);
  ...
  .delete("/documents/:id/tags/:tagId", async ({ params, set, request }) => {
    ...
    try {
      ...
      await db
        .delete(documentTags)
        .where(and(eq(documentTags.documentId, params.id), eq(documentTags.tagId, params.tagId)));
      return { success: true };
    } catch (err) {
