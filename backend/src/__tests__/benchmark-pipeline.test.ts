...
describe("pipeline benchmark report", () => {
	it("reports p50/p95, throughput, failures, fairness, and provider calls", () => {
		const report = buildPipelineBenchmarkReport([
			...
			{ ownerId: "a", queueWaitMs: 20, processingMs: 200, failed: true, providerCalls: 1 },
		...
		expect(report.fairness).toEqual({ ownerCount: 2, minCompleted: 1, maxCompleted: 1, spread: 0 });
