...
describe("createDocumentSchema", () => {
  test("accepts valid input with title and content", () => {
    const result = createDocumentSchema.safeParse({ title: "Test Doc", content: "Hello" });
...
describe("updateDocumentSchema", () => {
  ...
  test("accepts metadata object", () => {
    const result = updateDocumentSchema.safeParse({ metadata: { key: "value" } });
    expect(result.success).toBe(true);
  });
...
describe("createFolderSchema", () => {
  ...
  test("rejects invalid parentId UUID", () => {
    const result = createFolderSchema.safeParse({ name: "Sub", parentId: "not-uuid" });
    expect(result.success).toBe(false);
  });
});
...
describe("createTagSchema", () => {
  ...
  test("accepts optional color", () => {
    const result = createTagSchema.safeParse({ name: "important", color: "#ff0000" });
    expect(result.success).toBe(true);
  ...
  test("rejects color over 20 characters", () => {
    const result = createTagSchema.safeParse({ name: "tag", color: "x".repeat(21) });
    expect(result.success).toBe(false);
  });
});
