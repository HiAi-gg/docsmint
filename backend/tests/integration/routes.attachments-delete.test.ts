...
describe("DELETE /api/attachments/:id", () => {
	...
	it("commits the DB delete before attempting object cleanup", async () => {
		seedAttachment();
	...
	it("leaves the object untouched when the authoritative DB delete is fenced", async () => {
		seedAttachment();
		getState().accountPurgeFenceDeleteTables.add("attachments");
		const res = await request(app, `/api/attachments/${ATTACHMENT_ID}`, {
			method: "DELETE",
			headers: ownerHeaders(),
		});
		expect(res.status).toBe(409);
		expect(res.body).toMatchObject({ code: "ACCOUNT_PURGE_FENCED" });
