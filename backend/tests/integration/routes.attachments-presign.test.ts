...
describe("POST /api/documents/:id/attachments/presign", () => {
	it("returns 401 with an invalid bearer token (CSRF passes)", async () => {
		...
		const res = await request(app, `/api/documents/${docId}/attachments/presign`, {
			method: "POST",
			headers: {
...
describe("POST /api/documents/:id/attachments/confirm", () => {
	it("returns 401 with an invalid bearer token", async () => {
		const docId = seedOwnedDocument();
		const res = await request(app, `/api/documents/${docId}/attachments/confirm`, {
			method: "POST",
			headers: {
				authorization: "Bearer not-a-valid-key",
				"content-type": "application/json",
			...
			body: JSON.stringify(
				confirmBody({ documentId: docId, key: `${OWNER_ID}/${docId}/abc.png` }),
			),
		});
		expect(res.status).toBe(401);
	});
	...
	it("returns 400 when the key prefix doesn't match the document", async () => {
		...
		const res = await request(app, `/api/documents/${otherDoc}/attachments/confirm`, {
			method: "POST",
			headers: ownerHeaders(),
			body: JSON.stringify(
				confirmBody({
