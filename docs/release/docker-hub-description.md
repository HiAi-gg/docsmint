...
| --- | --- | --- | --- |
| API | `api-v0.8.8` | `api-latest` | Backend API and document processing |
| Web | `web-v0.8.8` | `web-latest` | Browser workspace |
| Caddy | `caddy-v0.8.8` | `caddy-latest` | Supporting reverse proxy with rate limiting |

**There is no bare `latest` tag.** Specify the service when pulling an image:

```bash
docker pull vgalibov/docsmint:api-v0.8.8
docker pull vgalibov/docsmint:web-v0.8.8
docker pull vgalibov/docsmint:caddy-v0.8.8
```

Caddy is the proxy component, not the DocsMint editor or API. Select `web-v0.8.8` for the browser workspace and `api-v0.8.8` for its backend.

The 0.8.8 images are published for **Linux amd64**. Use matching versioned tags
