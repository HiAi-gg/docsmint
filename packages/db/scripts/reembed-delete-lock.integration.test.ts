...
describe("re-embed delete parent-row lock integration", () => {
	it("blocks document and folder attaches until the locked parent is deleted", async () => {
		const setup = postgres(databaseUrl, { max: 1 });
		const folderDelete = postgres(databaseUrl, { max: 1 });
		const folderAttach = postgres(databaseUrl, { max: 1 });
		const categoryDelete = postgres(databaseUrl, { max: 1 });
		const categoryAttach = postgres(databaseUrl, { max: 1 });
