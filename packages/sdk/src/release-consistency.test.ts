...
const releaseVersion = "0.8.10";

async function json(path: string): Promise<Record<string, unknown>> {
  return JSON.parse(await readFile(new URL(path, repositoryRoot), "utf8"));
}

test("all published and workspace release metadata reports 0.8.10", async () => {
  for (const path of [
    "package.json",
    "package.public.json",
  ...
  for (const path of [
    ...
    expect(
      lockfile.workspaces[path]?.version,
  ...
  expect(
    (frontendManifest.dependencies as Record<string, string>)["lucide-svelte"],
  ).toBe("^1.0.1");
  ...
  expect(
    (registryManifest.packages as Array<{ version: string }>)[0].version,
  ).toBe(releaseVersion);
  ...
  ).toContain("docsmint-oss-0.8.10");
  expect(
    await readFile(new URL("docker-compose.yml", repositoryRoot), "utf8"),
  ).toContain("docsmint-oss-0.8.10");
});

test("workspace and public package metadata share the 0.8.10 product identity", async () => {
  ...
  for (const manifest of [workspaceManifest, publicManifest]) {
    ...
    expect(manifest.homepage).toBe(
      "https://docsmint.com/mcp/connect?source=npm_mcp",
    );
