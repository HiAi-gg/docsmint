...
test("all published and workspace release metadata reports 0.8.5", async () => {
  for (const path of [
    "package.json",
    "package.public.json",
  ...
  ).toContain("docsmint-oss-0.8.5");
  expect(
    await readFile(new URL("docker-compose.yml", repositoryRoot), "utf8"),
  ).toContain("docsmint-oss-0.8.5");
});

test("workspace and public package metadata share the 0.8.5 product identity", async () => {
