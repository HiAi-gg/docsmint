...
const releaseVersion = "0.8.9";

async function json(path: string): Promise<Record<string, unknown>> {
  return JSON.parse(await readFile(new URL(path, repositoryRoot), "utf8"));
}

test("all published and workspace release metadata reports 0.8.9", async () => {
  for (const path of [
    "package.json",
    "package.public.json",
  ...
  ).toContain("docsmint-oss-0.8.9");
  expect(
    await readFile(new URL("docker-compose.yml", repositoryRoot), "utf8"),
  ).toContain("docsmint-oss-0.8.9");
});

test("workspace and public package metadata share the 0.8.9 product identity", async () => {
