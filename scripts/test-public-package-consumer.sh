...
# Build a publish-shaped package, or consume the exact downloaded release
# tarball supplied by the provenance verifier, then exercise the advertised
# entrypoints from a clean Bun consumer. This never publishes or reads secrets.
repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
tmp_root=$(mktemp -d /tmp/docsmint-public-package.XXXXXX)
trap 'rm -rf "$tmp_root"' EXIT

if [[ -n "${DOCSMINT_PUBLIC_PACKAGE_TARBALL:-}" ]]; then
  tarball=$(realpath "$DOCSMINT_PUBLIC_PACKAGE_TARBALL")
  test -f "$tarball"
else
