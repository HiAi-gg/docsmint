...
const manifest = JSON.parse(await readFile(join(root, 'package.public.json'), 'utf8')) as {
  name: string;
  version: string;
  files: string[];
...
const packed = await run(
  ['bun', 'pm', 'pack', '--ignore-scripts', '--destination', tarballs, '--quiet'],
  stage
);
...
const listing = (await run(['tar', '-tzf', tarball], root)).stdout.split('\n').filter(Boolean);
...
const packagedBackend = await readFile(join(packageRoot, 'dist', 'backend', 'index.js'), 'utf8');
if (
...
for (const layer of ['--layer-chrome', '--layer-panel', '--layer-popover', '--layer-modal']) {
  if (!packagedFrontendStyles.includes(layer)) {
    throw new Error(`Packed frontend stylesheet is missing DocsMint layer contract: ${layer}`);
  }
}
...
for (const subpath of ssrComponentSubpaths) {
  ...
  const conditions = manifest.exports[exportPath] as Record<string, unknown> | undefined;
  if (
...
const packageLocalSvelteRuntimeChunks = packagedFrontendFiles
  .filter(
    ({ file, source }) => /\/client-[^/]+\.js$/.test(file) && source.includes('svelte/internal/')
  )
  .map(({ file }) => file);
if (packageLocalSvelteRuntimeChunks.length > 0) {
  throw new Error('Packed frontend facade bundles a package-local client runtime chunk');
}
if (!packagedFrontendFiles.some(({ source }) => source.includes('from "svelte/internal/client"'))) {
  throw new Error('Packed frontend facades do not retain the consumer Svelte runtime import');
}
...
const frontendPackage = JSON.parse(await readFile(join(root, 'frontend/package.json'), 'utf8')) as {
...
await writeFile(join(packageRoot, 'src/routes/+page.ts'), 'export const prerender = false;\n');
await writeFile(
...
`
  ...
  const browserBuild = await run(
    ['bun', viteBinary, 'build', '--config', config, '--configLoader', 'runner'],
    browserConsumer,
    false
  );
  if (browserBuild.exitCode === 0) {
    throw new Error(`Server-only ${check.subpath} export unexpectedly entered a browser bundle`);
  }
}
